...
int dfs_tmpfs_ioctl(struct dfs_fd *file, int cmd, void *args)
{
    ...
    d_file = (struct tmpfs_file *)file->data;
...
int dfs_tmpfs_write(struct dfs_fd *fd, const void *buf, size_t count)
{
    ...
    d_file = (struct tmpfs_file *)fd->data;
    RT_ASSERT(d_file != NULL);
...
int dfs_tmpfs_open(struct dfs_fd *file)
{
    ...
    /* find file */
    d_file = dfs_tmpfs_lookup(superblock, file->path, &size);
    ...
    if (file->flags & O_CREAT)
    {
        ...
        {
            /* find parent file */
            _path_separate(file->path, parent_path, file_name);
